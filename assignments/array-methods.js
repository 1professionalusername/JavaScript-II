...
runners.map(person => allCaps.push(person.first_name.toUpperCase()));
...
runners.filter(person => person.shirt_size === 'L').map(i => largeShirts.push(i));
...
let ticketPriceTotal = [];
    ...
    ticketPriceTotal.push(runners.reduce(acc, val) => (acc += val.donation), 0));
